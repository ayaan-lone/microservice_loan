...
public class LoanServiceImpl implements LoanService {
    ...
    public String issueLoan(Long userId, Double amount) throws LoanApplicationException {
        if (!isUserEligibleForLoan(userId)) {
            throw new LoanApplicationException(HttpStatus.NOT_FOUND, ConstantUtil.USER_NOT_FOUND);
        }
        if (isUserAlreadyLoaned(userId)) {
            throw new LoanApplicationException(HttpStatus.BAD_REQUEST, ConstantUtil.USER_ALREADY_LOANED);
        }
        // Logic to issue loan
        Loan loan = new Loan();
